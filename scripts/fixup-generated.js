 ...
 *
 * Additionally, some lexicons use types lex-cli can't process (e.g. permission-set).
 * These are excluded from codegen but injected into lexicons.ts schemaDict and ids
 * so runtime validation and ID lookup still work.
 *
 * This script:
 * - Builds a clean index.ts from discovered type files (no hardcoded list)
 * - Fixes missing .js import extensions
 * - Injects excluded lexicons into schemaDict and ids
 */
...
// Lexicons excluded from lex-cli codegen but needing schemaDict/ids entries.
// Each entry: { file: path relative to lexicons/, dictKey: schemaDict key }
const EXCLUDED_LEXICONS = [
  { file: 'forum/barazo/authForumAccess.json', dictKey: 'ForumBarazoAuthForumAccess' },
]
...
async function injectExcludedLexicons(lexiconsFile) {
  ...
  for (const { file, dictKey } of EXCLUDED_LEXICONS) {
    ...
    // Insert into schemaDict (before the closing `} as const satisfies`)
    ...
    // Insert into ids (before the closing `} as const`)
    // Find the ids block and insert alphabetically
...
async function main() {
  ...
  // Inject lexicons that lex-cli can't process
  await injectExcludedLexicons(join(GENERATED_DIR, 'lexicons.ts'))

  console.log(
    `Fixed ${typeFiles.length + 2} generated files (${typeFiles.length} types + lexicons.ts + util.ts)`
  )
}
